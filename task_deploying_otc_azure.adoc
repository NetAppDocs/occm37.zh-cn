...
+
最新权限位于中 https://mysupport.netapp.com/cloudontap/iampolicies["适用于 Azure 的 NetApp Cloud Central 策略"^]。
...
. * 定义工作环境 * ：选择 * Microsoft Azure* ，然后选择一个节点或 HA 对。
. * 详细信息和凭据 * ：可选择更改 Azure 帐户或订阅，指定集群名称和资源组名称，根据需要添加标记，然后指定凭据。
+
下表介绍了可能需要指导的字段：
...
| 交换机帐户 | 如果需要，您可以选择其他帐户或订阅 link:task_adding_azure_accounts.html["设置它们并将其添加到 Cloud Manager 中"]。 
...
|===
. * 服务 * ：如果您不想在此 Cloud Volumes ONTAP 系统中使用云合规性，请保持启用或禁用它。
+
